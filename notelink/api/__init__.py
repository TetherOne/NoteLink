...
from notelink.api.meee.users import router as meee_router


http_bearer = HTTPBearer(auto_error=False)
router = APIRouter(prefix=settings.api.full_prefix, dependencies=[Depends(http_bearer)])
router.include_router(notes_router)
router.include_router(tags_router)
router.include_router(auth_router)
router.include_router(meee_router)
